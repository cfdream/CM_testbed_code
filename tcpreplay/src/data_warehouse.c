...
int data_warehouse_init() {
    ...
    data_warehouse.flow_volume_map[a_idx] = ht_vi_create();
    if (data_warehouse.flow_volume_map[a_idx] == NULL) {
        return -1;
    }
    data_warehouse.flow_loss_volume_map[a_idx] = ht_vi_create();
    if (data_warehouse.flow_loss_volume_map[a_idx] == NULL) {
        return -1;
    }
    data_warehouse.flow_loss_rate_map[a_idx] = ht_vf_create();
    if (data_warehouse.flow_loss_rate_map[a_idx] == NULL) {
        return -1;
    }
    data_warehouse.target_flow_map[a_idx] = ht_kfs_create();
    if (data_warehouse.target_flow_map[a_idx] == NULL) {
        return -1;
    }
    data_warehouse.flow_sample_map[a_idx] = ht_kfs_create();
...
int data_warehouse_reset_noactive_buf() {
    ...
    ht_vi_destory(data_warehouse.flow_volume_map[na_idx]);
    ht_vi_destory(data_warehouse.flow_loss_volume_map[na_idx]);
    ht_kfs_destory(data_warehouse.target_flow_map[na_idx]);
    ht_vf_destory(data_warehouse.flow_loss_rate_map[na_idx]);
    ht_kfs_destory(data_warehouse.flow_sample_map[na_idx]);
    //recreate the hashmaps
    data_warehouse.flow_volume_map[na_idx] = ht_vi_create();
    if (data_warehouse.flow_volume_map[na_idx] == NULL) {
        return -1;
    }
    data_warehouse.flow_loss_volume_map[na_idx] = ht_vi_create();
    if (data_warehouse.flow_loss_volume_map[na_idx] == NULL) {
        return -1;
    }
    data_warehouse.flow_loss_rate_map[na_idx] = ht_vf_create();
    if (data_warehouse.flow_loss_rate_map[na_idx] == NULL) {
        return -1;
    }
    data_warehouse.target_flow_map[na_idx] = ht_kfs_create();
    if (data_warehouse.target_flow_map[na_idx] == NULL) {
        return -1;
    }
    data_warehouse.flow_sample_map[na_idx] = ht_kfs_create();
