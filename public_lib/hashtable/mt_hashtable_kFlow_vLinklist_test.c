...
void *test_set_func(void* param_ptr) {
	...
	hashtable_vl_t *hashtable = (hashtable_vl_t*) param_ptr;
    int i = 0;
    //for (i = 1; i < 10; ++i) {
    int j = 1;
    for (j = 1; j < 1000001*50; j++) {
        ...
        for (i = 1; i < 2; i++) {
            flow.srcip = i;
            ht_vl_set(hashtable, &flow, j, DEFAULT_PKT_VOLUME); 
        }
    }
...
void *test_get_func(void* param_ptr) {
    ...
    printf("oldest_pkt seqid:%u\n", hashtable->table[ht_vl_hash(hashtable, &flow)]->oldest_pkt->seqid);
    printf("newest_pkt seqid:%u\n", hashtable->table[ht_vl_hash(hashtable, &flow)]->newest_pkt->seqid);
