...
void *test_set_func(void* param_ptr) {
    flow_s flow;
	hashtable_vi_t *hashtable = (hashtable_vi_t*) param_ptr;
    int i = 0;
    //for (i = 1; i < 10; ++i) {
    while(1) {
        int i = 1;
...
void *test_get_func(void* param_ptr) {
	...
	hashtable_vi_t *hashtable = (hashtable_vi_t*) param_ptr;
    while(1) {
        int value = ht_vi_get(hashtable, &flow);
...
void *test_next_func(void* param_ptr) {
    ...
    while(1) {
        int cnt = 0;
        while (ht_vi_next(hashtable, &entry) == 0) {
            ++cnt;
        }
        printf("next_cnt:%d\n", cnt);
    }
...
int main( int argc, char **argv ) {
    ...
    flow_s flow;
    flow.srcip = 1;
    ht_vi_set(hashtable, &flow, 1);
    flow.srcip = 2;
    ht_vi_set(hashtable, &flow, 2);
    flow.srcip = 3;
    ht_vi_set(hashtable, &flow, 3);
    int i = 1;
    for (i = 1; i < 65536*3; i++) {
        flow.srcip = i;
        ht_vi_set(hashtable, &flow, i);
    ...
    flow.srcip = 1;
    printf("get flow key-%d, value-%d\n", flow.srcip, ht_vi_get(hashtable, &flow));
